...
export default function ImageCarousel({
  ...
  if (total === 0) {
    return (
      <div className="w-full h-48 bg-white/[0.03] flex items-center justify-center rounded-t-lg">
  ...
  return (
    <div
      className="relative w-full h-48 overflow-hidden rounded-t-lg bg-white/[0.03] group/carousel"
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-[#0f1115] to-transparent pointer-events-none" />
        ...
        <>
          <button
            ...
            className="absolute left-2 top-1/2 -translate-y-1/2 z-10
                       w-7 h-7 rounded-full bg-black/50 border border-white/10
                       flex items-center justify-center
                       opacity-0 group-hover/carousel:opacity-100
                       hover:bg-black/80 hover:border-[#c8973a]/40
      ...
      {/* Image counter badge */}
      {total > 1 && (
        <div className="absolute top-2 right-2 z-10 px-2 py-0.5 rounded-full bg-black/50 border border-white/10 text-[10px] text-white/50 font-medium">
          {current + 1}/{total}
        </div>
      )}
    </div>
